/**
 * サンプル食品データ投入スクリプト
 * MEXT CSV がなくても動作確認できる最小限のデータ
 * 使い方: npx tsx scripts/seed-foods.ts
 */
...
const FOODS = [
  ...
  // 飲料
  { code: "16-001", name: "コーヒー（インスタント・浸出液）", cat: "嗜好飲料類", kcal: 4, p: 0.2, f: 0, c: 0.7 },
  { code: "16-002", name: "緑茶（玉露・浸出液）", cat: "嗜好飲料類", kcal: 5, p: 0.6, f: 0.1, c: 0.5 },
  // 調味料
...
const SERVINGS: Array<[string, Array<[string, number, number]>]> = [
...
function seed() {
  ...
  const insertFood = db.prepare(`
    INSERT OR REPLACE INTO GenericFood (
      id, foodCode, nameJa, category, caloriesKcal,
      proteinG, fatG, carbG, source, createdAt
    ) VALUES (
  ...
  const insertAlias = db.prepare(`
    INSERT OR IGNORE INTO FoodAlias (id, alias, genericFoodId)
    SELECT lower(hex(randomblob(16))), ?, id FROM GenericFood WHERE foodCode = ?
  `);

  const insertServing = db.prepare(`
    INSERT OR IGNORE INTO FoodServing (id, genericFoodId, servingName, servingG, sortOrder)
    SELECT lower(hex(randomblob(16))), id, ?, ?, ? FROM GenericFood WHERE foodCode = ?
  `);

  const insertFts = db.prepare(`
    INSERT OR REPLACE INTO foods_fts (food_id, food_type, name_ja, name_en, category)
    SELECT id, 'generic', nameJa, NULL, category FROM GenericFood WHERE foodCode = ?
  `);

  const tx = db.transaction(() => {
    for (const f of FOODS) {
      insertFood.run(f.code, f.name, f.cat, f.kcal, f.p, f.f, f.c);
      // デフォルト100g
      insertServing.run("100g", 100, 0, f.code);
      insertFts.run(f.code);
    }
    for (const [code, aliases] of ALIASES) {
      for (const alias of aliases) {
        insertAlias.run(alias, code);
      }
    }
    for (const [code, servings] of SERVINGS) {
      for (const [name, g, order] of servings) {
        insertServing.run(name, g, order, code);
      }
    }
  });
  ...
  tx();
  db.close();
  console.log(`✅ ${FOODS.length} 件の食品データを投入しました`);
}
